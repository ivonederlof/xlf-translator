...
class XlfHandler {
     ...
     */
    createXlfWorkSheet() {
        return Promise.all(global_1.Global.config.languageFileNames.map(fileName => {
            const path = [global_1.Global.config.output, constants_1.Constants.XLF_OUTPUT_DIRECTORY, fileName];
            return file_util_1.FileUtil.copyFile([global_1.Global.config.source], path).then(fileBuffer => new xlf_message_model_1.XlfMessage(path, fileBuffer));
        })).then(messages => file_util_1.FileUtil.readFile([global_1.Global.config.source]).then(source => {
            const sourceMessage = new xlf_message_model_1.XlfMessage([global_1.Global.config.source], source);
