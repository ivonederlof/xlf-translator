...
XlfProcessor.prototype.translateFileFromManualCsvDirectory = function (callback) {
    ...
    async.waterfall([
        (callback) => {
            xlfFileProcessor.listFiles('./files/translations/csv/', (err, files) => {
                callback(err, files)
            })
        },
        (files, callback) => {
...
XlfProcessor.prototype.validateAndChangeTargetsIfNeeded = function (file, callback) {
    ...
    async.waterfall([
        (callback) => {
            xlfFileProcessor.readAndParseCsvToJson(`./files/translations/csv/${file}`, (err, csvTranslationsArray) => {
        ...
        (csvTranslationsArray, callback) => {
            ...
            xlfFileProcessor.readXlfFile(`./files/messages/${fileName}.xlf`, (err, xlfFile) => {
                messageFile = xlfFile;
