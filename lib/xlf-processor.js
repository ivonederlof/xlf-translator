...
XlfProcessor.prototype.checkAndUpdateMessagesIfAvailableFromSourceForTranslations = function (done) {
    ...
    async.waterfall([
        ...
        (callback) => {
            ...
            let newMessages = [];
            const firstFileSources = firstFileMessages.map((firstFileMessage) => firstFileMessage.$.id);

            sourceMessages.forEach((sourceMessage, index) => {
                const exstingIndex = firstFileSources.indexOf(sourceMessage.$.id);
                if (exstingIndex === -1) {
                    newMessages.push(sourceMessages[index]);
                }
                // const existingTranslation = firstFileMessages.find((firstFileMessage) => sourceMessage.source[0] === firstFileMessage.source[0]);
            ...
            xlfFileProcessor.listFiles(messageFilesDirPath, (err, files) => {
                callback(err, files, newMessages);
            })
        },
        (files, newMessages, callback) => {
