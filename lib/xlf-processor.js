...
XlfProcessor.prototype.translateAndProcessFilesWithGoogleApi = function (toLanguages, callback) {
    ...
    async.waterfall([
        ...
        (xlfFileAsString, callback) => {
            ...
            async.forEach(toLanguages, (languageToTranslate, next) => {
    ...
    async.waterfall([
        ...
        (callback) => {
            ...
            if (sourceMessages.length === firstFileMessages.length) {
                console.log(chalk.gray('No updates found in source file\n'));
                return done();
            }
...
XlfProcessor.prototype.translateFilesFromManualCsvTranslations = function (callback) {
    ...
    async.waterfall([
        ...
        if (totalMutated > 0) {
            console.log(chalk.gray(`${totalMutated} files updated\n`));
        } else {
            console.log(chalk.gray('No updates made in csv files\n'));
        }
        callback(err);
    });
};
