...
XlfProcessor.prototype.translateFileWithGoogleApi = function (callback) {
    ...
    async.waterfall([
        ...
        (callback) => {
            const xlfFilePath = `${appRoot}${translatorConfig.source}/${constants.SOURCE_FILE_NAME}.${constants.FILE_TYPE}`;
            // get the file string
            ...
            async.forEach(translatorConfig.toLanguage, (languageToTranslate, next) => {
                ...
                async.waterfall([
                    ...
                    (newBody, callback) => {
                        ...
                        xlfFileProcessor.createXlfFile(`${appRoot}${translatorConfig.outputPath}/${constants.OUTPUT_FILE_NAME}/${constants.OUTPUT_FILE_NAME}.${languageToTranslate}.${constants.FILE_TYPE}`, xml, (err) => {
                            callback(err);
                        });
                    }
                ], (err) => {
